...
import { collection, query,deleteDoc, onSnapshot, addDoc, orderBy, getDocs, serverTimestamp, updateDoc, doc, arrayUnion, getDoc ,arrayRemove } from "firebase/firestore";
import { signOut } from "firebase/auth";
import { useNavigate } from "react-router-dom";
import { Container, Row, Col, ListGroup, Form, Button, Card, Dropdown, Modal } from "react-bootstrap";
import { FaUserPlus ,FaEllipsisV, FaTrash, FaUserMinus ,FaArrowLeft } from "react-icons/fa";
 ...
 const deleteMessage = async (messageId, forEveryone = false) => {
  ...
  const removeContact = async (userId) => {
   ...
   const deleteChat = async () => {
     ...
     const snapshot = await getDocs(messagesRef); // Fetch messages once
    ...
    <Button variant="light" size="sm" className="me-2" onClick={() => setSelectedUser(null)}>
      <FaArrowLeft />
                    ...
                    <Form onSubmit={sendMessage} className="p-3 border-top d-flex">
                      {replyTo && <small className="text-muted">Replying to: {replyTo.text} <span onClick={() => setReplyTo(null)} style={{ cursor: "pointer", color: "red" }}>×</span></small>}
